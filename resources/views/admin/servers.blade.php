...
<div class="overflow-auto h-screen pb-24 px-4 md:px-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-4 w-full">
                ...
                <form action="{{ route('admin.servers.store') }}" method="post" id="create-role">
                    @csrf
                        <div class="mb-3 space-y-2 w-full text-xs">
                            <label class="font-semibold text-gray-500 py-2">BattleMetrics ID <abbr title="required">*</abbr></label>
                            <input class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-grey-lighter rounded-lg h-10 px-4" required="required" type="text" id="bm_id" name="bm_id" value="{{old('bm_id')}}">
        ...
        <div class="flex flex-col col-span-2 bg-white w-full shadow-lg rounded p-4">
            ...
            <div class="flex items-center relative p-2 w-full bg-white rounded-lg overflow-hidden shadow hover:shadow-md border-solid border-2 border-sky-500 mb-2 flow-root">
                <div class="ml-3 my-auto float-left">
                    <p class="font-medium text-black">{{ $server['data']['attributes']['name'] }}</p>
                </div>
