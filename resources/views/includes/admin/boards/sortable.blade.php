    ...
    @foreach($boardsCol->where('category_id', $cat->id) as $board)
        @if(isset($boards[$board->id]))
            <div class="list-group-item" data-id="{{$board->id}}" data-category-id="{{$board->category_id}}">
                <div class="flex items-center relative p-2 w-1/2 bg-white dark:bg-gray-600 rounded-lg overflow-hidden shadow hover:shadow-md border-solid border-2 border-[{{$board->color}}] mb-2 flow-root">
                    <div class="ml-3 float-left">
                        <p class="font-medium text-black dark:text-white">{{ $board->name }}</p>
                    </div>
